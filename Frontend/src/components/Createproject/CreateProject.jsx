...
export default function CreateProject({ onProjectCreated }) {
  ...
  function handleDeleteRow(idToDelete) {
    const updatedRows = rows.filter(row => row.id !== idToDelete);
    setRows(updatedRows);
  }
  ...
  function handleCreate() {
    ...
    // After creating the project, call the onProjectCreated prop
    onProjectCreated();
  }
  ...
  return (
    <div className='createproject'>
      ...
      <TextField className='budget' label="Budget" variant="outlined" type="number" />
      <hr />
      <InputLabel className="projectteam text-black" id="projectteam-label">Project Team</InputLabel>
      {rows.map((item, index) => (
        <div key={item.id}>
          <FormControl sx={{ m: 1, ml: 4, mt: 2, minWidth: 186 }}>
            <InputLabel id={`job-role-label-${item.id}`}>Job role</InputLabel>
            <Select
              labelId={`job-role-label-${item.id}`}
              id={`job-role-${item.id}`}
              label="Job role"
            >
              <MenuItem value="tpm">TPM</MenuItem>
      ...
      <Link className='addnewemp text-black text-decoration-none' onClick={handleAddRow}>+ Add new Employee</Link>
      <hr />
      <Button className="createproject-btn float-end" variant="contained" onClick={handleCreate}>Create</Button>
    </div>
  );
}