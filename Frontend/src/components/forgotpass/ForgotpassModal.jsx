...
//     e.preventDefault();
//     try {
//       const response = await axios.post('https://ems-api.antiers.work/api/forgotpassword', { email });
...
export default function ForgotpassModal({ handleClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://ems-api.antiers.work/api/forgot-password', { email });
      toast.success(response.data.message);
      navigate('/');
    } catch (error) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <TextField
          ...
          onChange={handleChange}
        />
        <Button className="forgotpass m-4" variant="contained" type="submit">
          Submit
        </Button>
      </form>
    </>
